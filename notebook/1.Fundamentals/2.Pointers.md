...
You can imagine a variable as a conveyor of a content (value) with a position in memory (address).
In other words, each value is stored in an address in memory. This address can be long and unpractical to use, so the variable is just another name given to that location.
